...
import { SchichtzuweisungViewDialog } from '@/components/dialogs/SchichtzuweisungViewDialog';
...
const MITARBEITER_FIELDS = [
  ...
  { key: 'position', label: 'Position/Rolle', type: 'string/text' },
...
const SCHICHTVORLAGEN_FIELDS = [
  ...
  { key: 'anzahl_mitarbeiter', label: 'Benötigte Anzahl Mitarbeiter', type: 'number' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
];
const SCHICHTZUWEISUNG_FIELDS = [
  { key: 'datum', label: 'Datum', type: 'date/date' },
  { key: 'schichtvorlage', label: 'Schichtvorlage', type: 'applookup/select', targetEntity: 'schichtvorlagen', targetAppId: 'SCHICHTVORLAGEN', displayField: 'schichtname' },
...
const ENTITY_TABS = [
  ...
  { key: 'schichtzuweisung', label: 'Schichtzuweisung', pascal: 'Schichtzuweisung' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichtzuweisung': return (data as any).schichtzuweisung as Schichtzuweisung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'schichtzuweisung' && fieldKey === 'schichtvorlage') {
      const match = (lists.schichtvorlagenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.schichtname ?? '—';
    }
    if (entity === 'schichtzuweisung' && fieldKey === 'mitarbeiter') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichtzuweisung': return SCHICHTZUWEISUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichtzuweisung': return {
        create: (fields: any) => LivingAppsService.createSchichtzuweisungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichtzuweisungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichtzuweisungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'schichtzuweisung' || dialogState?.entity === 'schichtzuweisung') && (
        <SchichtzuweisungDialog
          open={createEntity === 'schichtzuweisung' || dialogState?.entity === 'schichtzuweisung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichtzuweisung' ? handleUpdate : (fields: any) => handleCreate('schichtzuweisung', fields)}
          defaultValues={dialogState?.entity === 'schichtzuweisung' ? dialogState.record?.fields : undefined}
          ...
          enablePhotoLocation={AI_PHOTO_LOCATION['Schichtzuweisung']}
        />
      )}
      ...
      {viewState?.entity === 'schichtzuweisung' && (
        <SchichtzuweisungViewDialog
          open={viewState?.entity === 'schichtzuweisung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichtzuweisung', record: r }); }}
          schichtvorlagenList={(data as any).schichtvorlagen ?? []}
          mitarbeiterList={(data as any).mitarbeiter ?? []}
        />
      )}
